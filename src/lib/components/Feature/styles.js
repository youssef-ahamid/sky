...
export let feature = options => {
  return [
    {
      classes:
        'relative w-80 md:w-[500px] h-64 md:h-72 z-0 focus-within:z-30 hover:z-30 my-4 md:mx-4 md:my-16 mx-auto cursor-pointer group focus-visible:outline-none transition',
    },
  ]
}
...
export let otherImage = options => {
  return [
    {
      classes: 'absolute transition duration-500 ease-out z-0 opacity-0 w-80 md:w-[500px] h-48 md:h-60 overflow-hidden object-contain object-center group-focus-within:delay-200 group-hover:delay-200 group-focus-within:z-10 group-hover:z-10 ',
    },
    {
      classes: '',
      on: [options.active, true]
...
export let cta = options => {
  return [
    {
      classes: 'my-4 opacity-0 hidden group-focus-within:block group-focus-within:opacity-100 group-hover:block group-hover:opacity-100',
    },
  ]
}
export let description = options => {
  return [
    {
      classes: 'w-72 md:w-[400px] line-clamp-3 opacity-0 hidden group-focus-within:block group-focus-within:opacity-100 group-hover:block group-hover:opacity-100 font-medium',
    },
  ]
}
let options
