...
export let title = options => {
  return [
    {
      classes: 'select-none text-neutral-light text-4xl md:text-5xl md:translate-y-1 -translate-x-4 one-liner absolute left-8 md:left-10 uppercase transition duration-300 ease-out origin-top-left w-[70%] leading-8 scale-[50%] top-8 z-20 text-left group-focus-within:scale-[60%] group-hover:scale-[60%] md:max-w-[150px]',
    },
    {
      classes: 'scale-[60%]',
      on: [options.active, true]
...
export let cta = options => {
  return [
    {
      classes: 'my-4 group-focus-within:block group-focus-within:opacity-100 group-hover:block group-hover:opacity-100',
    },
    {
...
export let description = options => {
  return [
    {
      classes: 'w-full line-clamp-3 group-focus-within:block group-focus-within:opacity-100 group-hover:block group-hover:opacity-100 font-medium',
    },
    {
      classes: 'block opacity-100',
      on: [options.active, true]
